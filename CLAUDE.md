...
`src/` = `globals.css`(Tailwind v4 入口)+ `lib/utils.ts`(`cn()`)+ `hooks/`(app)+ **`design-system/`**(`README.md` + `hooks/` / `tokens/` / `components/` / `patterns/`,各 dir charter 必先讀)+ `explorations/`(未定案 prototype)。Internal vs public 分類見 `components/README.md` 及 `# Story`。**Token**:純 CSS(無 JS),`color/` `typography/` `uiSize/` `layoutSpace/` `opacity/` `radius`;初始 `<html data-theme="light" data-density="md">`;動態 `setAttribute('data-theme', 'dark')`;JS 端 `var(--token-name)` 字串。完整檔案清單 + dark/density 切換 → `src/design-system/tokens/README.md`。
...
**正式 vs 比稿**:`src/design-system/` 已定案 / `src/explorations/` 未定案 prototype;正式 code 禁 import explorations(hook `block_prototype_imports.py` 強制)。Exploration:每題 `src/explorations/{topic}/` folder + `*.v1.stories.tsx` / `notes.md`;定案 → 升 `patterns/`(runtime primitive)或 `components/`。**Skills**:`/prototype`(user 明言「prototype/MVP/原型」走,5 phases + Phase 3.5 audit gate);`/component-quality-gate`(merge 前 45 項 Spec/Code/Stories/Ship checklist)。
...
**三層定位**(深度遞進 看→查→判斷):**展示** `{name}.stories.tsx`(設計規格便利瀏覽,trait-based v2)/ **設計規格** `{name}.anatomy.stories.tsx`(token + 尺寸藍圖,取代 Figma Inspect,5-canonical)/ **設計原則** `{name}.principles.stories.tsx`(do/don't + 情境選擇,Polaris-aligned 4-section canonical;universal core ≥ 2 of `WhenToUse`/`WhenNotToUse`/`Vs*Rule`/`ContentGuidelines`,2026-04-26)。
...
**範例最高準則**:精簡幹練、0 重複、每 story earn its existence(audit Dim 24/25/28/29/30 抓)。**Earn-existence 2 test**(兩題皆 NO → retire):(a) 教別 story 沒教的原則?(b) 移除後 spec 理解 degrade?**拆分原則**(對齊 Polaris / Carbon / Storybook):不同 affordance 必分 / AllVariants & AllSizes 對照各 1 / 同 affordance 內 prop variations 用 Controls 不另開(❌ `WithStartIcon`+`WithEndIcon` → ✓ `WithIcon` grid)/ Compound 有 new constraint 才分。**展示 v2 trait-based**:spec.md frontmatter `traits:` array → required core stories 衍生 + hook `check_story_category.sh` 攔。**Principles canonical**(Polaris-aligned):universal core ≥ 2 of `WhenToUse`/`WhenNotToUse`/`Vs*Rule`/`ContentGuidelines` + hook `check_principles_canonical.sh` 攔。SSOT → `/story-writing` skill `references/category-templates.md`。

❌ 禁止:佔位符 / 抽象代號 / 極端不現實 / 視覺符號 / spec 內部代號。詳 → `/story-writing` skill。
