...
## Overlay:Status dot / Count badge(兩角、兩語義)

Avatar 支援兩個 overlay indicator,**走不同角、管不同事、可同時存在**。

| Overlay | Prop | 位置 | 語義 | 世界級對照 |
|---------|------|------|------|-----------|
| Status dot(presence)| `status="online"/"away"/"busy"/"offline"` | **右下角**(circle avatar 圓周 45°) | 此人現在線上狀態 | Slack / Teams / Discord |
| Count badge(通知)| `badgeCount={N}` | **右上角** | 此對話 / 此使用者有 N 條新事件 | iMessage / Slack thread / LINE / WhatsApp |
