...
| 間距 | 值 | 來源 canonical |
|------|-----|--------------|
| **Label ↔ Description gap** | `mt-0.5`(2px) | `patterns/element-anatomy/item-anatomy.spec.md`「Label ↔ Desc 間距」(同 Empty 實踐,DS-wide 一致) |
...
### Type A 跟 Type B **不混合共存**(重要 invariant)

**Type A completed**(100% bar + ✓)**不會**跟 Type B(無 bar)在同一 list 共存:
- Type A completed 是「剛完成的 upload session」視覺,屬 upload manager 情境
- Type B 是「已存 attachment」視覺,屬 form attachment 情境
- 兩者業務語義互斥 —— 完成後 consumer 會把 item 轉成 Type B(移除 status 屬性)

**合法 mixed 情境**(email 草稿 / 多步驟 upload flow):
- Type A **active**(`status="uploading"` / `status="error"`)+ Type B(saved attachments)
- 一旦 Type A 完成 → 轉成 Type B,不會同時顯示「completed with bar」+「無 bar」

---

## List wrapper canonical(多 item 間距 · 2026-04-23 簡化)

`FileItem` 連續排列時 list wrapper gap **統一規則**:

| Mode | List wrapper gap | Rationale |
|------|----------------|-----------|
| **Rich** | `gap-2`(8px) | card 邊框不相黏(standalone card invariant) |
| **Compact**(所有情境) | `gap-1`(4px) | **統一** — 不論 Type A only / Type B only / mixed,都用 gap-1 簡化 canonical。避免 Type A 無 bg 的 0-gap 變 consumer 心智負擔 |

**先前版本**(已簡化):compact Type A 純列 0 gap / Type B 或 mixed gap-1。現統一為 **compact 永遠 gap-1**(user 2026-04-23 指示簡化)。

---

## List wrapper canonical(多 item 間距 · 原版說明保留)
