...
```
[Avatar 56 square    [ label
 固定,top-align       ↓ mt-0.5 (2px)
 作視覺引導]           desc (可 wrap)   ] [suffix h-[1lh]]
                      ↓ gap-2 (8px explicit)
                     [ ██████████░░ ]
```

**Avatar 對齊的明文例外(上游 item-anatomy 24px 規則的例外)**:
...
Avatar **固定 56px square**,**不隨 content 高度變化**。content(label + desc + bar)可自然 > 56(2-line desc 時)或 < 56(無 desc + 無 bar 時),avatar 始終 56。
