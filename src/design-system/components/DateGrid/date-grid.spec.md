...
| State | 視覺 | Token | 備註 |
|-------|------|-------|------|
| **today**(未選) | 數字下方短圓桿(藍) | `::after` bar:`bottom-[5px]` `w-[40%]` `h-[1.5px]` `rounded-full` `bg-primary` | 不 fill bg,affordance 方式;`bottom-[5px]` 刻意貼近數字(不貼 button 邊) |
| **today + selected** | 數字下方短圓桿(白)| bar 同上,`bg` 切 `on-emphasis` | 選中藍底上藍 bar 隱形,必切白;透過 `[&[data-selected=true]>button]:after:bg-on-emphasis` 覆寫 |
| **disabled** | 灰底 + 淡字(跟 Button disabled 同調) | `bg-disabled text-fg-disabled cursor-not-allowed` | 跟 Button disabled token 一致,不自創 palette |
| **outside(非本月)**| 淡字(只文字) | `text-fg-muted`(neutral-7) | 比 disabled 弱:outside 仍可 hover / 可點,純是「非焦點月份」的標示 |
| **selected / range 端點** | 藍底白字圓 | `[&>button]:bg-primary [&>button]:text-on-emphasis`(cell `data-selected="true"`) | range_start / range_end 共用此視覺 |
| **range 端點 cell bg** | 灰底半圓 track | range_start: `rounded-l-full bg-[var(--color-neutral-3)]`;range_end: `rounded-r-full` 同 | 半圓 track 讓端點圓跟中段 rectangle 無縫連通 |
| **range track(中間)** | 灰底矩形 | `bg-[var(--color-neutral-3)]` on cell;button `!bg-transparent` 顯露 track | neutral-3 比 neutral-2 對比度清楚(light mode 才能被看見) |
| **hover(未選中)** | 藍圈 outline(無 fill) | `hover:ring-[1.5px] hover:ring-primary hover:bg-transparent` | 1.5px ring 對齊 Apple HIG / Ant 慣例 |
