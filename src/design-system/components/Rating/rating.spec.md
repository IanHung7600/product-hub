...
| Size | Icon 尺寸 | 使用情境 | 對齊對象 |
|------|-----------|---------|---------|
| `sm` | 16px | 商品列表、搜尋結果、DataTable cell 內的星等展示 | Checkbox/Radio `sm` 控件 16px |
| `md` | 16px | **預設**。一般卡片、評論列表、商品詳情頁 | Checkbox/Radio `md` 控件 16px |
| `lg` | 20px | 送出評分的 review form、強調的主 CTA 區塊 | Checkbox/Radio `lg` 控件 20px |

### Size scale 對齊 Checkbox / Radio

Rating 的 `sm / md / lg` 尺寸一律與 **Checkbox / Radio** 的 control box 同尺寸(16 / 16 / 20),理由:

1. **視覺 baseline 一致**:Rating 常與 Checkbox / Radio 在同一 form 出現(例如「整體滿意度 Rating + 是否推薦 Radio」),icon 大小對齊讓每一列控件視覺穩定,不刻度錯亂
2. **Field 塞入免調整**:Rating 包進 `<Field>` 時(見下方範例),控件高度與 Checkbox/Radio 一致,Field 不需為 Rating 特別撐行高
3. **對齊 Lucide icon 標準尺寸**:16 / 20 都是 DS 常用 icon 尺寸(見 `patterns/element-anatomy/item-anatomy.spec.md` ICON_SIZE)

**md 刻意 = sm**(16px):對齊 Checkbox sm=md=16 的慣例(field height 跨 sm/md 共享 16px control 的設計決策)。想要更大的顯示 rating 用 `lg`。
...
Field 高度由 Rating icon 自然撐起,**Rating 本體不需綁 `--field-height-*`**(它是 visual indicator,非 editable single-line input)。這是與 Field 家族共存但不繼承 field-height 的 canonical 案例——`aria-invalid` 透過 FieldContext 自動傳入,視覺錯誤提示由 FieldError 承擔。
...
| Focus ring | `ring-2 ring-ring ring-offset-2` + `rounded-md` | — | 鍵盤 focus 時整個 Rating 容器顯示 focus ring |
