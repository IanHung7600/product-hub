...
**Close X 按鈕(2026-04-22 v3:overlay family canonical 統一)**:所有 `PopoverHeader` 內建右上 X 按鈕,**size="sm"** 跟 Dialog / Sheet / Coachmark 統一(overlay family canonical — 詳 `patterns/overlay-surface/overlay-surface.spec.md`「Chrome dismiss size canonical」)。實作 = `<Button iconOnly dismiss size="sm" startIcon={X} aria-label="關閉" />`(非 Inline Action / 非自刻 button)。`hideClose` prop 可讓 composition 元件(如 Coachmark)選擇隱藏。

**Header / Footer 高度 canonical**:透過 `SurfaceHeader` / `SurfaceFooter` primitive 提供的 `min-h-[var(--chrome-header-height)]`(48 md / 56 lg),sm button 置中於 chrome slot,視覺 height = chrome-header-height canonical。跟 Dialog / Sheet 完全一致 — **overlay family chrome 高度 + button size 全統一**,無元件特化。density 鎖 md 仍是 popover 獨有 rationale(content area 輕量呼吸),但 chrome typography / button size 跟 overlay family 走同 canonical。
