...
**Close X 按鈕(2026-04-20 決策)**:所有 `PopoverHeader` 內建右上 X 按鈕(對齊 Dialog canonical)—— consumer 只需渲染 `<PopoverHeader>title 內容</PopoverHeader>`,X 自動出現並綁 Radix Close。有 header 的 Popover 通常結構化程度高(title / 多區塊),明確的關閉入口讓使用者更易退出。無 header 的簡單 Popover 不加 X(click-outside / Esc 即可)。
