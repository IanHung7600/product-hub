...
**偏離 canonical 規則**:Consumer 若有明確合理理由不用 primitive / 自訂 label-desc typography,**必在該元件 `spec.md` 明文 rationale**。已知合法偏離:
- **MenuItem**:`leading-compact + text-caption`(scanning mode typography 特化)— v2 可遷 `<ItemContent mode="scanning">`
- **SelectionItem**:control slot 跟 block formula 綁定
- **Steps**:step indicator + connector line pattern
